  ...
  return (
    <section className={`${classes.bracket_container}`}>
      ...
      <div className={`${classes.bracket_control_container}`}>
        <MyButton onClick={() => {copyToClipboard()}} additionalCl={`${classes.copy_button}`}>Copy link</MyButton>
        <div className={`${classes.bracket_warning}`}>This bracket will be deleted over time to save the bracket create a tournament.</div>
      </div>
    </section>
  );
};
