            ...
            multi_stage = MultiStage(
                clear_participants(validated_data.get("participants")),
                {
                    ...
                    "advance_from_group": int(
                        self.initial_data.get("advance_from_group")
                    ),
                    "type": self.initial_data.get("type"),
                    "group_type": self.initial_data.get("group_type"),
                },
                {
                    "time_managment": json.loads(
                        self.initial_data.get("time_managment")
                    ),
                    ...
                    "final_stage_time": json.loads(
                        self.initial_data.get("final_stage_time")
                    ),
                },
                {
            ...
            tournament = Tournament.objects.create(
                **validated_data,
                owner=Profile.objects.get(
                    user__email=self.initial_data.get("creater_email")
                ),
            )
            ...
            if self.initial_data.get("type") == "SE":
                single_el = SingleEl(
                    clear_participants(validated_data.get("participants")),
                    {
                        ...
                        "start_time": validated_data.get("start_time"),
                        "avg_game_time": int(self.initial_data.get("avg_game_time")),
                        "max_games_number": int(
                            self.initial_data.get("max_games_number")
                        ),
                        "break_between": int(self.initial_data.get("break_between")),
                        "mathes_same_time": int(
                            self.initial_data.get("mathes_same_time")
                        ),
                    },
                    json.loads(self.initial_data.get("secod_final")),
                )
    ...
    def create(self, validated_data):
        ...
        if validated_data.get("type") == "SE":
            ...
            bracket = Bracket.objects.create(
                bracket=single_el.create_se_bracket(), type=validated_data.get("type")
            )
            ...
            bracket = Bracket.objects.create(
                bracket=double_el.create_de_bracket(), type=validated_data.get("type")
            )
            ...
            bracket = Bracket.objects.create(
                bracket=swiss.create_swiss_bracket(), type=validated_data.get("type")
            )
